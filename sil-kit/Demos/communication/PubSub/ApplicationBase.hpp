...
struct PerformanceStats
{
    ...
    Clock::time_point simulationStartWall;
    Clock::time_point simulationEndWall;

    Clock::time_point initializationStart;
    Clock::time_point initializationEnd;

    Clock::time_point terminationStart;
    Clock::time_point terminationEnd;

    std::chrono::nanoseconds firstSimulationTime{0};
    std::chrono::nanoseconds lastSimulationTime{0};
    ...
    void StartProcess()
    {
        ...
        initializationStart = processStart;
    }

    void StartSimulation(std::chrono::nanoseconds simulationTime)
    {
        if (!simulationStarted)
        {
            simulationStarted = true;
            simulationStartWall = Clock::now();
            firstSimulationTime = simulationTime;
        }
    }

    void BeginStep(std::chrono::nanoseconds simulationTime)
    {
    ...
    template<typename F>
    void MeasureExecution(std::chrono::nanoseconds simulationTime, F&& fn)
    {
        BeginStep(simulationTime);
    ...
    void EndSimulation()
    {
        simulationEndWall = Clock::now();
    }

    void EndInitialization()
    {
        initializationEnd = Clock::now();
    }

    void StartTermination()
    {
        terminationStart = Clock::now();
    }

    void EndTermination()
    {
        terminationEnd = Clock::now();
    }
};
...
void PrintPerformanceStats()
{
    ...
    const auto wallclockSimulation =
        duration_cast<nanoseconds>(
            _performance.simulationEndWall -
            _performance.simulationStartWall);

    const auto initialization =
        duration_cast<nanoseconds>(
            _performance.initializationEnd -
            _performance.initializationStart);

    auto execution =
        _performance.executionTime;

    auto nonExecution =
        wallclockSimulation - execution;

    if (nonExecution.count() < 0)
    {
        nonExecution = nanoseconds{0};
    }

    const auto simulationDuration =
        _performance.lastSimulationTime -
        _performance.firstSimulationTime;

    const double wallclockSeconds =
        duration<double>(wallclockSimulation).count();

    const double simulationSeconds =
        duration<double>(simulationDuration).count();

    const double executionSeconds =
        duration<double>(execution).count();

    const double initializationSeconds =
        duration<double>(initialization).count();

    const double idleSeconds =
        duration<double>(nonExecution).count();
    ...
    std::cout
        ...
        << "execution.count="
        << _performance.executionCount << '\n'

        << "simulation.steps="
        << _performance.simulationStepCount << '\n';

    if (simulationSeconds > 0.0)
    {
        std::cout
            << "real.time.factor="
            << simulationSeconds / wallclockSeconds
            << '\n';
    }
    ...
    void SetupLifecycle()
    {
        ...
        _lifecycleService->SetCommunicationReadyHandler([this]() {
            ...
            if (!_arguments.runAutonomous)
            {
                ...
                {
                    _hasSystemControllerResult = true;
                    _waitForSystemControllerPromise.set_value(
                        SystemControllerResult::CoordinatedStart);
                }
            }

            InitControllers();

            _performance.EndInitialization();
        });
    ...
    void SetupSync()
{
    ...
    _timeSyncService->SetSimulationStepHandler(
        ...
        {
            _performance.StartSimulation(now);

            _performance.MeasureExecution(
                now,
                [this, now]()
                {
                    std::stringstream ss;
    ...
    void WaitUntilDone()
    {
        ...
        if (_performance.simulationStarted)
            {
                _performance.EndSimulation();
            }
